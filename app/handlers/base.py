...
async def cmd_help(message: types.Message, user: User):
    ...
    text = [
        ...
        _("{command} - Show list of your lots").format(command="/mylots"),
        _("{command} - Publish new lots").format(command="/upload"),
        _("{command} - Delete your lots").format(command="/delete"),
    ]
    await message.reply("\n".join(text))
